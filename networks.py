...
class MsImageDis(nn.Module):
    ...
    #     # for i in range(2):
    #     #     cnn_x += [ResBlock(dim, norm=self.norm, activation=self.activ, pad_type=self.pad_type)] 
    #     # if self.non_local>0:
    #     #     cnn_x += [NonlocalBlock(dim)]
    #     cnn_x += [nn.Conv2d(dim, 1, 1, 1, 0)]
    #     cnn_x = nn.Sequential(*cnn_x)
    ...
    def calc_dis_loss(self, input_fake, input_real):
        ...
        for it, (out0, out1) in enumerate(zip(outs0, outs1)):
            if self.gan_type == 'lsgan':
                ...
                reg += 0.01 * self.compute_grad2(out1, input_real).mean()
...
class AdaINGen(nn.Module):
    ...
    def __init__(self, input_dim, params):
        ...
        # smooth filter
        self.sf = ResBlocks(num_blocks=4, dim=3, norm='none', activation=activ, pad_type=pad_type)
    ...
    def encode(self, images, mask, texture):
        ...
        mask = (mask + 1) / 2
        add_mask0 = mask.repeat(1, 3, 1, 1) * images
        #add_mask1 = ((1 - mask) * images.mean(dim=1, keepdim=True)).repeat(1, 3, 1, 1)
        new_images = add_mask0 - (texture*2*(0.8+0.4*random.random())).repeat(1,3,1,1)#+ add_mask1
        content = self.enc_content(new_images)
        return content, style_fake, new_images

    def decode(self, content, style, shortcut):
        # decode content and style codes to an image
        adain_params = self.mlp(style)
        self.assign_adain_params(adain_params, self.dec)
        # shortcut = self.sf(shortcut)
        images = self.dec(content, shortcut)
        return images
...
class Decoder(nn.Module):
    def __init__(self, n_upsample, n_res, dim, output_dim, res_norm='adain', activ='relu', pad_type='zero'):
        ...
        self.model = nn.Sequential(*self.model)
        self.final_conv += [nn.Conv2d(3, 3, kernel_size=7, stride=1, padding=3)]
        # self.final_conv += [nn.ReLU()]
        self.final_conv = nn.Sequential(*self.final_conv)

    def forward(self, x, x_shortcut):
        return self.final_conv(self.model(x) + x_shortcut)
